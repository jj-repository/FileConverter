...
from app.utils.validation import validate_file_size, validate_file_extension, validate_download_filename, validate_mime_type, ARCHIVE_MIME_TYPES
...
async def convert_archive(
    ...
    try:
        ...
        validate_mime_type(input_path, ARCHIVE_MIME_TYPES)
...
async def get_archive_info(file: UploadFile = File(...)):
    ...
    try:
        ...
        if 'temp_path' in locals():
            cleanup_file(temp_path)
        raise HTTPException(status_code=500, detail=f"Failed to get archive info: {sanitize_error_message(str(e))}")
