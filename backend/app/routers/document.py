...
async def convert_document(
    ...
    try:
        ...
        output_path = await document_converter.convert_with_cache(
...
async def get_document_info(file: UploadFile = File(...)):
    ...
    try:
        ...
        if 'temp_path' in locals():
            cleanup_file(temp_path)
        raise HTTPException(status_code=500, detail=f"Failed to get document info: {sanitize_error_message(str(e))}")
