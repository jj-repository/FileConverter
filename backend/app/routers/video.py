...
from app.utils.validation import validate_download_filename, validate_file_size, validate_file_extension, validate_mime_type, VIDEO_MIME_TYPES
...
async def convert_video(
    ...
    try:
        ...
        validate_mime_type(input_path, VIDEO_MIME_TYPES)
...
async def get_video_info(file: UploadFile = File(...)):
    ...
    try:
        ...
        if 'temp_path' in locals():
            cleanup_file(temp_path)
        raise HTTPException(status_code=500, detail=f"Failed to get video info: {sanitize_error_message(str(e))}")
