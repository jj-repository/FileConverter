    ...
    async def convert(
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_extract_dir:
                ...
                await self.send_progress(
                    session_id, 60, "converting", f"Creating {output_format} archive"
                )
    ...
    def _sync_extract_archive(self, archive_path: Path, extract_to: Path, format: str):
        ...
        if format == "zip":
                ...
                for member in tf.getmembers():
                    ...
                    if member.issym() or member.islnk():
                        ...
                        if link_target.startswith("/") or ".." in link_target:
                            raise ValueError(
                                f"Unsafe symlink target: {member.name} -> {link_target}"
                            )
                        ...
                        try:
                            ...
                            raise ValueError(
                                f"Symlink escapes extraction directory: {member.name} -> {link_target}"
                            )
                    # Verify the resolved path stays within extract directory
            ...
            with py7zr.SevenZipFile(archive_path, mode="r") as archive:
                ...
                # Extract only validated members
                archive.extract(path=extract_to, targets=safe_names)

        else:
            raise ValueError(f"Unsupported format for extraction: {format}")

    async def _create_archive(
        self, source_dir: Path, output_path: Path, format: str, compression_level: int
    ):
