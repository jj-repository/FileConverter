    ...
    async def convert_with_cache(
        ...
        try:
            ...
            cache_key = await asyncio.to_thread(
                cache_service.generate_cache_key, input_path, output_format, options
            )
                ...
                if cached_file != output_path:
                    ...
                    async with _cache_locks_lock:
                        ...
                        if cache_key not in _cache_locks:
                            ...
                            _cache_locks[cache_key] = (
                                _cache_locks[cache_key][0],
                                current_time,
                            )
                    ...
                    async with file_lock:
                        ...
                        try:
                            if cached_file.exists():
                                ...
                                logger.warning(
                                    f"Cached file {cached_file} no longer exists, reconverting"
                                )
                            ...
                            logger.warning(
                                f"Cached file {cached_file} was deleted during copy, reconverting"
                            )
                            return await self.convert(
                                input_path, output_format, options, session_id
                            )
    ...
    async def send_progress(
        ...
        if self.websocket_manager:
            await self.websocket_manager.send_progress(
                session_id=session_id, progress=progress, status=status, message=message
            )

    def validate_format(
        self, input_format: str, output_format: str, supported_formats: Dict[str, list]
    ) -> bool:
        """Validate that input and output formats are supported"""
        return input_format in supported_formats.get(
            "input", []
        ) and output_format in supported_formats.get("output", [])
