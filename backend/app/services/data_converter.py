...
class DataConverter(BaseConverter):
    ...
    async def convert(
        ...
        await self.send_progress(
            session_id, 0, "converting", "Starting data conversion"
        )
        ...
        try:
                ...
                with open(input_path, "r", encoding=encoding) as f:
                    for line_num, line in enumerate(f, 1):
                        ...
                        if len(line) > MAX_LINE_LENGTH:
                            raise ValueError(
                                f"Line {line_num} exceeds maximum length ({MAX_LINE_LENGTH // 1024 // 1024}MB)"
                            )
                        if line.strip():
            ...
            await self.send_progress(
                session_id, 60, "converting", "Converting data format"
            )
            ...
            await self.send_progress(
                session_id, 100, "completed", "Data conversion completed"
            )
    ...
    async def _dataframe_to_xml(
        self, df: pd.DataFrame, output_path: Path, encoding: str
    ):
