...
class TestArchiveSecurityValidation:
    ...
    def test_archive_with_path_traversal_members_blocked(
        self, client, malicious_zip_with_path_traversal
    ):
...
class TestArchiveInfo:
    ...
    def test_get_archive_info_zip_success(self, client, sample_zip):
        ...
        with open(sample_zip, "rb") as f:
            response = client.post(
                "/api/archive/info", files={"file": ("test.zip", f, "application/zip")}
            )
    ...
    def test_get_archive_info_tar_gz_success(self, client, temp_dir):
        ...
        with open(tar_path, "rb") as f:
            response = client.post(
                "/api/archive/info", files={"file": ("test.tar.gz", f, "application/gzip")}
            )
    ...
    def test_get_archive_info_invalid_format(self, client, temp_dir):
        ...
        with open(invalid_file, "rb") as f:
            response = client.post(
                "/api/archive/info", files={"file": ("test.txt", f, "text/plain")}
            )

        # Validation returns 400 for invalid extension
        assert response.status_code == 400
    ...
    def test_get_archive_info_cleanup_on_error(self, client, temp_dir, monkeypatch):
        ...
        with patch(
            "app.services.archive_converter.ArchiveConverter.get_archive_info",
