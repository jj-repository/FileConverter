            ...
            with patch.object(converter, "send_progress", new=AsyncMock()) as mock_progress:
                with patch("asyncio.create_subprocess_exec") as mock_subprocess:
                    ...
                    async def _mk_output(*args, **kwargs):
                        ...
                        _out_idx = args.index("-o") + 1 if "-o" in args else -1
                        _P(args[_out_idx]).write_bytes(b"fake")
                        return mock_process
            ...
            with patch.object(converter, "send_progress", new=AsyncMock()) as mock_progress:
                with patch("asyncio.create_subprocess_exec") as mock_subprocess:
                    ...
                    mock_process.communicate = AsyncMock(
                        return_value=(b"", b"Pandoc error: invalid input")
                    )
        ...
        with patch("subprocess.run") as mock_check:
            ...
            with patch("subprocess.run") as mock_pandoc:
                ...
                mock_pandoc.return_value = Mock(
                    returncode=0, stdout="Test Document Some content here"
                )
    ...
    async def test_get_metadata_pdf_metadata_error_handled(self, temp_dir):
        ...
        with patch("subprocess.run") as mock_check:
            ...
            with patch("pypdf.PdfReader") as mock_reader:
                ...
                type(mock_pdf.metadata).title = PropertyMock(
